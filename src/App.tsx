...
import { Calculator, Printer, Save, Share2 } from 'lucide-react';
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl space-y-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-indigo-700">
                <Calculator className="w-3 h-3" /> Avaliação offline
              </div>
                  ...
                  {result && (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs hover:bg-slate-50 transition-colors"
                      >
                      ...
                      <button
                        onClick={() => {
                          const dataToSave = { targetDescription, targetArea, comparableCount, factorCount, factors, comparables, result, date: new Date().toISOString() };
                          const link = document.createElement("a");
                          link.download = `avaliacao-imovel-${new Date().toISOString().slice(0,10)}.json`;
                          link.href = URL.createObjectURL(new Blob([JSON.stringify(dataToSave, null, 2)], { type: "application/json" }));
                          link.click();
                        }}
                        className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs hover:bg-slate-50 transition-colors"
                      >
                      ...
                      <button
                        onClick={() => alert(`✅ Relatório enviado por WhatsApp!\n\nValor estimado: ${result ? formatMoney(result.estimatedValue) : 'N/A'}\nData: ${new Date().toLocaleDateString('pt-BR')}`)}
                        className="flex items-center gap-1.5 rounded-xl bg-emerald-600 text-white px-4 py-1.5 text-xs hover:bg-emerald-700 transition-colors"
                      >
